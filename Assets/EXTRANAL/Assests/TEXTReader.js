...
function speakText(text, voiceName = changeDefaultFemaleVoice()) {
    const outputSpeech = document.getElementById('outputSpeech');
...
function changeDefaultFemaleVoice() {
    const speechSynthesis = window.speechSynthesis;
    const voices = speechSynthesis.getVoices();
    let desiredVoice = null;

    // Find a female-sounding voice
    desiredVoice = voices.find(voice => {
        return voice.name.toLowerCase().includes('female');
    });

    // If no female voice is found, find any voice with a female attribute
    if (!desiredVoice) {
        desiredVoice = voices.find(voice => voice.gender === 'female');
    }

    if (desiredVoice) {
        speechSynthesis.onvoiceschanged = null; // Remove event listener to prevent potential conflicts
        speechSynthesis.cancel(); // Cancel any current speech synthesis

        // Set the desired voice as the default
        speechSynthesis.defaultVoice = desiredVoice;
    } else {
